	...
	{	Scanner sc = new Scanner(System.in);
		...
		String bip = "";
			...
			{	split_bip[i] = appendZeros(Integer.toBinaryString(Integer.parseInt(split_ip[i])));
				bip += split_bip[i];	}
		System.out.println("\n \t Binary Format "+bip);
		System.out.print("\n \t Enter the number of addresses in each subnet: ");
		int n = sc.nextInt();  //Calculation of mask
		int bits = (int)Math.ceil(Math.log(n)/Math.log(2));
		int mask = 32-bits;
		System.out.println("\n \t Subnet mask = "+mask); //Calculation of first address and last address
		int fbip[] = new int[32];
		...
		for(int i=31;i>31-bits;i--)/*Get first address by ANDing last n bits with 0 */
		 fbip[i] &= 0; String fip[] = {"","","",""}; 
		for(int i=0;i<32;i++)
			fip[i/8] = new String(fip[i/8]+fbip[i]); System.out.print("\n \t Network address is = ");	
		for(int i=0;i<4;i++){
		System.out.print(Integer.parseInt(fip[i],2)); if(i!=3) System.out.print("."); }
		System.out.println();
		int lbip[] = new int[32];
